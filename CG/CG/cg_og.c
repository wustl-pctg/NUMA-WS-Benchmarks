...
c-------------------------------------------------------------------*/
    ...
    int *colidx_numa = (int *) pattern_bind_memory_numa(colidx_pages, 4, mem_pattern);
    ...
    int *rowstr_numa = (int *) pattern_bind_memory_numa(rowstr_pages, 4, mem_pattern);
    ...
    double *x_numa = (double *) pattern_bind_memory_numa(x_pages, 4, mem_pattern);
    ...
    double *z_numa = (double *) pattern_bind_memory_numa(z_pages, 4, mem_pattern);
    cilk_for(int i =0;i < NA+2+1; i++){
      z_numa[i] = z[i];
    }
    ...
    double *a_numa = (double *) pattern_bind_memory_numa(a_pages, 4, mem_pattern);
    cilk_for(int i =0;i < rNZ+1; i++){
      a_numa[i] = a[i];
    }
    //p
    unsigned long long p_pages = (NA+2+1)*sizeof(double)/getpagesize()+1;
    double *p_numa = (double *) pattern_bind_memory_numa(p_pages, 4, mem_pattern);
    cilk_for(int i =0;i < NA+2+1; i++){
      p_numa[i] = p[i];
    }
    ...
    double *q_numa = (double *) pattern_bind_memory_numa(q_pages, 4, mem_pattern);
    cilk_for(int i =0;i < NA+2+1; i++){
      q_numa[i] = q[i];
    }
    ...
    double *r_numa = (double *) pattern_bind_memory_numa(r_pages, 4, mem_pattern);
    cilk_for(int i =0;i < NA+2+1; i++){
      r_numa[i] = r[i];
    }
...
/* 	} */
	...
	double norm_temp11_1 = cilk_spawn reduce_add_mul(norm_temp11, x_numa, z_numa, 1, midt11); // @0
	SET_PIN(pin_pattern[1]);
	double norm_temp12_1 = cilk_spawn reduce_add_mul(norm_temp12, z_numa, z_numa, 1, midt11); // @0
	double norm_temp11_2 = cilk_spawn reduce_add_mul(norm_temp11, x_numa, z_numa, midt11, midt1); // @1
	SET_PIN(pin_pattern[2]);
	double norm_temp12_2 = cilk_spawn reduce_add_mul(norm_temp12, z_numa, z_numa, midt11, midt1); // @1
	double norm_temp11_3 = cilk_spawn reduce_add_mul(norm_temp11, x_numa, z_numa, midt1, midt12); // @2
	SET_PIN(pin_pattern[3]);
	double norm_temp12_3 = cilk_spawn reduce_add_mul(norm_temp12, z_numa, z_numa, midt1, midt12); // @2
	double norm_temp11_4 = cilk_spawn reduce_add_mul(norm_temp11, x_numa, z_numa, midt12, lastcol-firstcol+2); // @3
	// __cilkrts_unset_pinning_info();
	__cilkrts_enable_nonlocal_steal();
	double norm_temp12_4 = reduce_add_mul(norm_temp12, z_numa, z_numa, midt12, lastcol-firstcol+2); // @3
        SET_PIN(pin_pattern[0]);
	cilk_sync;
	norm_temp11 = norm_temp11_1 + norm_temp11_2 + norm_temp11_3 + norm_temp11_4;
	norm_temp12 = norm_temp12_1 + norm_temp12_2 + norm_temp12_3 + norm_temp12_4;
...
c-------------------------------------------------------------------*/
	...
	__cilkrts_disable_nonlocal_steal();
	SET_PIN(pin_pattern[1]);
	cilk_spawn map_mul_scalar(x_numa, z_numa, norm_temp12, 1, midt11); // @0
	SET_PIN(pin_pattern[2]);
        cilk_spawn map_mul_scalar(x_numa, z_numa, norm_temp12, midt11, midt1); // @1
	SET_PIN(pin_pattern[3]);
	cilk_spawn map_mul_scalar(x_numa, z_numa, norm_temp12, midt1, midt12); // @2
	// __cilkrts_unset_pinning_info();
	__cilkrts_enable_nonlocal_steal(); 
	map_mul_scalar(x_numa, z_numa, norm_temp12, midt12, lastcol-firstcol+2); // @3
	SET_PIN(pin_pattern[0]);
	cilk_sync;
...
//{
    ...
    /* 	//w[j] = 0.0; */
    /* } */
    unsigned long init_mid = (1 + naa+2)/2;
    unsigned long init_mid1 = (1 + init_mid)/2;
    unsigned long init_mid2 = (init_mid + naa+2)/2;
    __cilkrts_disable_nonlocal_steal();
    SET_PIN(pin_pattern[1]);
    cilk_spawn initialize(q, z, r, p, x, 1, init_mid1); // @0
    SET_PIN(pin_pattern[2]);
    cilk_spawn initialize(q, z, r, p, x, init_mid1, init_mid); // @1
    SET_PIN(pin_pattern[3]);
    cilk_spawn initialize(q, z, r, p, x, init_mid, init_mid2); // @2
    // __cilkrts_unset_pinning_info();
    __cilkrts_enable_nonlocal_steal(); 
    initialize(q, z, r, p, x, init_mid2, naa+2); // @3
    SET_PIN(pin_pattern[0]);
    cilk_sync;
/*--------------------------------------------------------------------
c  rho = r.r
...
/* 	rho = rho + r[j]*r[j]; */
/*     } */
    unsigned long mid = (1 + lastrow-firstrow+2)/2;
    unsigned long mid1 = (1 + mid)/2;
    unsigned long mid2 = (mid + lastrow-firstrow+2)/2;
    //printf("mid: %d, mid1: %d, mid2: %d, e: %d\n", mid, mid1, mid2, lastrow-firstrow+2);
    __cilkrts_disable_nonlocal_steal();
    SET_PIN(pin_pattern[1]);
    double rho1 = cilk_spawn reduce_add_mul(rho, r, r, 1, mid1);
    SET_PIN(pin_pattern[2]);
    double rho2 = cilk_spawn reduce_add_mul(rho, r, r, mid1, mid);
    SET_PIN(pin_pattern[3]);
    double rho3 = cilk_spawn reduce_add_mul(rho, r, r, mid, mid2);
    // __cilkrts_unset_pinning_info();
    __cilkrts_enable_nonlocal_steal(); 
    double rho4 = reduce_add_mul(rho, r, r, mid2, lastrow-firstrow+2);
    SET_PIN(pin_pattern[0]);
    cilk_sync;
...
/* rolled version */    
      ...
      __cilkrts_disable_nonlocal_steal();
      SET_PIN(pin_pattern[1]);
      cilk_spawn compute_q(rowstr, colidx, p, q, a, 1, mid1);
      SET_PIN(pin_pattern[2]);
      cilk_spawn compute_q(rowstr, colidx, p, q, a, mid1, mid);
      SET_PIN(pin_pattern[3]);
      cilk_spawn compute_q(rowstr, colidx, p, q, a, mid, mid2);
      // __cilkrts_unset_pinning_info();
      __cilkrts_enable_nonlocal_steal();
      compute_q(rowstr, colidx, p, q, a, mid2, lastrow-firstrow+2);
      SET_PIN(pin_pattern[0]);
      cilk_sync;
...
/* #pragma omp barrier */
      __cilkrts_disable_nonlocal_steal();
      SET_PIN(pin_pattern[1]);
      double d1 = cilk_spawn reduce_add_mul(d, p, q, 1, mid1);
      SET_PIN(pin_pattern[2]);
      double d2 = cilk_spawn reduce_add_mul(d, p, q, mid1, mid);
      SET_PIN(pin_pattern[3]);
      double d3 = cilk_spawn reduce_add_mul(d, p, q, mid, mid2);
      // __cilkrts_unset_pinning_info();
      __cilkrts_enable_nonlocal_steal(); 
      double d4 = reduce_add_mul(d, p, q, mid2, lastcol-firstcol+2);
      SET_PIN(pin_pattern[0]);
      cilk_sync;
      d = d1 + d2 + d3 + d4;
/*--------------------------------------------------------------------
...
//#pragma omp for reduction(+:rho)	
	...
	/* } */
	__cilkrts_disable_nonlocal_steal();
	cilk_spawn map_add_mul(z, z, p, alpha, 1, mid1); // @0
	SET_PIN(pin_pattern[1]);
	cilk_spawn map_add_mul(r, r, q, -alpha, 1, mid1); // @0
	cilk_spawn map_add_mul(z, z, p, alpha, mid1, mid); // @1
	SET_PIN(pin_pattern[2]);
	cilk_spawn map_add_mul(r, r, q, -alpha, mid1, mid); // @1
	cilk_spawn map_add_mul(z, z, p, alpha, mid, mid2); // @2
	SET_PIN(pin_pattern[3]);
	cilk_spawn map_add_mul(r, r, q, -alpha, mid, mid2); // @2
	cilk_spawn map_add_mul(z, z, p, alpha, mid2, lastcol-firstcol+2); // @3
	// __cilkrts_unset_pinning_info();
	__cilkrts_enable_nonlocal_steal(); 
	map_add_mul(r, r, q, -alpha, mid2, lastcol-firstcol+2); // @3
	SET_PIN(pin_pattern[0]);
	cilk_sync;
/*---------------------------------------------------------------------
c  rho = r.r
...
//#pragma omp barrier
	__cilkrts_disable_nonlocal_steal();
	SET_PIN(pin_pattern[1]);
	double rho_1 = cilk_spawn reduce_add_mul(rho, r, r, 1, mid1);
	SET_PIN(pin_pattern[2]);
	double rho_2 = cilk_spawn reduce_add_mul(rho, r, r, mid1, mid);
	SET_PIN(pin_pattern[3]);
	double rho_3 = cilk_spawn reduce_add_mul(rho, r, r, mid, mid2);
	// __cilkrts_unset_pinning_info();
	__cilkrts_enable_nonlocal_steal(); 
	double rho_4 = reduce_add_mul(rho, r, r, mid2, lastcol-firstcol+2);
	SET_PIN(pin_pattern[0]);
	cilk_sync;
...
//#pragma omp for nowait
	...
	__cilkrts_disable_nonlocal_steal();
	SET_PIN(pin_pattern[1]);
	cilk_spawn map_add_mul(p, r, p, beta, 1, mid1);
	SET_PIN(pin_pattern[2]);
	cilk_spawn map_add_mul(p, r, p, beta, mid1, mid);
	SET_PIN(pin_pattern[3]);
	cilk_spawn map_add_mul(p, r, p, beta, mid, mid2);
	// __cilkrts_unset_pinning_info();
	__cilkrts_enable_nonlocal_steal(); 
	map_add_mul(p, r, p, beta, mid2, lastcol-firstcol+2);
	SET_PIN(pin_pattern[0]);
	cilk_sync;
...
c---------------------------------------------------------------------*/
    ...
    __cilkrts_disable_nonlocal_steal();
    SET_PIN(pin_pattern[1]);
    cilk_spawn compute_norm(rowstr, colidx, a, z, r, 1, mid1);
    SET_PIN(pin_pattern[2]);
    cilk_spawn compute_norm(rowstr, colidx, a, z, r, mid1, mid);
    SET_PIN(pin_pattern[3]);
    cilk_spawn compute_norm(rowstr, colidx, a, z, r, mid, mid2);
    // __cilkrts_unset_pinning_info();
    __cilkrts_enable_nonlocal_steal(); 
    compute_norm(rowstr, colidx, a, z, r, mid2, lastrow-firstrow+2);
    SET_PIN(pin_pattern[0]);
    cilk_sync;
/*--------------------------------------------------------------------
...
/* } */
    __cilkrts_disable_nonlocal_steal();
    SET_PIN(pin_pattern[1]);
    double sum1 = cilk_spawn compute_sum(x, r, 1, mid1);
    SET_PIN(pin_pattern[2]);
    double sum2 = cilk_spawn compute_sum(x, r, mid1, mid);
    SET_PIN(pin_pattern[3]);
    double sum3 = cilk_spawn compute_sum(x, r, mid, mid2);
    // __cilkrts_unset_pinning_info();
    __cilkrts_enable_nonlocal_steal(); 
    double sum4 = compute_sum(x, r, mid2, lastcol-firstcol+2);
    SET_PIN(pin_pattern[0]);
    cilk_sync;
